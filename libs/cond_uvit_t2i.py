...
class Cond_UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.hint_embed = zero_module(nn.Linear(clip_dim, embed_dim))
    ...
    def forward(self, x, timesteps, context, hint):
        ...
        class_token = self.hint_embed(hint)
        ...
        class_token = class_token + context_token
        x_cls = torch.cat((time_token, class_token, x_copy), dim=1)
        # del x_copy
        x_cls = x_cls + self.pos_embed

        controls = []
        for blk, zero_linear in zip(self.in_blocks, self.zero_linears):
            with torch.no_grad():
                x_cls = blk(x_cls)
            controls.append(zero_linear(x_cls))

        with torch.no_grad():
            x_cls = self.mid_block(x_cls)
        x_cls = self.middle_block_out(x_cls)      
