...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.prior_embed = nn.Linear(clip_dim, embed_dim)

        self.extras = 1 + 1 + num_clip_token
    ...
    def forward(self, x, timesteps, context, prior):
        ...
        prior_token = self.prior_embed(prior)

        x = torch.cat((time_token, context_token, prior_token, x), dim=1)
        x = x + self.pos_embed
