...
def main():
    ...
    model = get_model('/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/stable-diffusion/autoencoder_kl.pth')
    # model = get_model('/storage/ScientificPrograms/Diffusion/code/autoencoder_pretrain/pretrain_v1/Jun29_14-09-49/1-0.0008.pth')
    ...
    #     transforms.RandomResizedCrop(size=(32, 32), scale=(0.8, 1.2), ratio=(0.8, 1.2)),
    #     # transforms.ColorJitter(brightness=0.05, contrast=0.5, saturation=0.05, hue=0.05),
    # ])

    # paths = ['/storage/ScientificPrograms/Diffusion/data/MIMIC_CXR_JPG/MIMIC-CXR-JPG/files/p10/p10000032/s50414267/02aa804e-bde0afdd-112c0b34-7bc16630-4e384014.jpg']
    paths = ['/storage/ScientificPrograms/Diffusion/data/MIMIC_CXR_JPG/MIMIC-CXR-JPG/files/p10/p10000935/s56522600/f1adcae3-2921c0a8-5d9652f9-4191ecd7-f2a96f35.jpg']
    for p in paths:
        ...
        with torch.cuda.amp.autocast():
            ...
            recons = [model.decode(model.encode(img)) for _ in range(1)]

        out = torch.cat([img, *recons], dim=0)
        out = (out + 1) * 0.5
        save_image(out, f'debug.png')
