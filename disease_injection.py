...
def train(config):
    ...
    def cfg_nnet(x, timesteps, context, hint):
        ...
        _uncond = nnet_ema(x, timesteps, context=_empty_context, hint = _empty_context)
    ...
    def train_step(_batch):
        ...
        loss = LSimple(_z, nnet, _schedule, context=_batch[1], hint=_batch[2])  # currently only support the extracted feature version
    ...
    def eval_step(n_samples, sample_steps):
        ...
        def sample_fn(_n_samples):
            _context, _hint = next(context_generator)
            assert _context.size(0) == _n_samples
            return dpm_solver_sample(_n_samples, sample_steps, context=_context, hint=_hint)
    ...
    while train_state.step < config.train.n_steps:
        ...
        if accelerator.is_main_process and train_state.step % config.train.eval_interval == 0:
            ...
            hints = torch.tensor(dataset.hints, device=device)[: 2 * 5]
            samples = dpm_solver_sample(_n_samples=2 * 5, _sample_steps=50, context=contexts, hint=hints)
