...
class CFGDataset_prior(Dataset):  # for classifier free guidance
    def __init__(self, dataset, p_uncond, empty_token):
        self.dataset = dataset
        self.p_uncond = p_uncond
        self.empty_token = empty_token
...
class MSCOCOFeatureDataset_CL(Dataset):
    # the image features are got through sample
    def __init__(self, root=None, stage=None, mode=None):
        self.root = root
        self.stage = stage
        self.mode = mode
        if mode == 'val':
            ...
            self.files = []
            while index < stage:
                with open(f'/storage/ScientificPrograms/Diffusion/data/MIMIC_CXR_JPG/A_TXT_FILES/curriculum_len_npy_{index + 1}.txt', 'r') as f:
                    self.files = self.files + f.readlines()
                f.close()
                index = index + 1
            
            self.files = [file.strip() for file in self.files]
            self.num_data, self.n_captions = len(self.files), len(self.files)
    ...
    def __getitem__(self, index):
        if self.mode == 'val':
            ...
            z = np.load(self.files[index])
            c = np.load(self.files[index][:-4] + '_0.npy')
        return z, c


class MSCOCOFeatureDataset_prior(Dataset):
    # the image features are got through sample
    def __init__(self, root):
        self.root = root
        self.num_data, self.n_captions = get_feature_dir_info(root)
    ...
    def __getitem__(self, index):
        ...
        k = random.randint(0, self.n_captions[index] - 1)
        c = np.load(os.path.join(self.root, f'{index}_0.npy'))
        p = np.load(os.path.join(self.root, f'{index}_prior.npy'))
        return z, c, p


class MSCOCOFeatureDataset_fine(Dataset):
    # the image features are got through sample
    def __init__(self, root):
        self.root = root
        self.num_data, self.n_captions = get_feature_dir_info(root)
...
class NLMCXR256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset(os.path.join(path, 'train'))
    ...
    @property
    def fid_stat(self):
        return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_NLMCXR.npz'

class MIMIC256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset(os.path.join(path, 'train'))
    ...
    def fid_stat(self):
        ...
        return f'assets/fid_stats/MIMIC_frontal_all_final.npz'

class MIMIC256Features_CL(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None, stage=1):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset_CL(path, stage, 'train')
        self.test = MSCOCOFeatureDataset_CL(os.path.join(path, 'val'), stage, 'val')
        print('Prepare dataset ok')
    ...
    def fid_stat(self):
        # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_MIMIC_all.npz'
        # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/MIMIC_frontal_all_final.npz'
        return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/MIMIC_frontal_CL.npz'
...
#     def fid_stat(self):
#         # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_MIMIC_all.npz'
#         return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/cls_MIMIC_frontal_all.npz'


class MIMIC256Features_fine(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset_fine(os.path.join(path, 'train'))
        self.test = MSCOCOFeatureDataset_fine(os.path.join(path, 'val'))
        print('Prepare dataset ok')
...
class MIMIC256Features_test(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset(os.path.join(path, 'train'))
    ...
    @property
    def fid_stat(self):
        # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_MIMIC_test.npz'
        return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/MIMIC_frontal_all_final.npz'


class MIMIC256Features_prior(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset_prior(os.path.join(path, 'train'))
        self.test = MSCOCOFeatureDataset_prior(os.path.join(path, 'val'))
        print('Prepare dataset ok')
        ...
        if cfg:  # classifier free guidance
            assert p_uncond is not None
            self.empty_context = np.load(os.path.join(path, 'empty_context.npy'))
            # self.empty_prior = np.load(os.path.join(path, 'empty_context.npy'))
            print(f'prepare the dataset for classifier free guidance with p_uncond={p_uncond}')
            self.train = CFGDataset_prior(self.train, p_uncond, self.empty_context)

        # text embedding extracted by clip
        # for visulization in t2i
        self.prompts, self.contexts, self.hints = [], [], []
        for f in sorted(os.listdir(os.path.join(path, 'run_vis')), key=lambda x: int(x.split('.')[0])):
            # prompt, context = np.load(os.path.join(path, 'run_vis', f), allow_pickle=True)
            # self.prompts.append(prompt)
            # self.contexts.append(context)
            ...
            self.prompts.append(data.item()['prompt'])
            self.contexts.append(data.item()['latent'])
            self.hints.append(data.item()['hint'])
        self.contexts = np.array(self.contexts)
        self.hints = np.array(self.hints)
...
class ISIC256Features_prior(DatasetFactory):  
    def __init__(self, path, cfg=False, p_uncond=None):
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset_prior(os.path.join(path, 'train'))
        self.test = MSCOCOFeatureDataset_prior(os.path.join(path, 'val'))
    ...
    def fid_stat(self):
        # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_MIMIC_test.npz'
        return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_ISIC.npz'



class Colorectal256Features_prior(DatasetFactory):  
    def __init__(self, path, cfg=False, p_uncond=None):
        print('Prepare dataset...')
        self.train = MSCOCOFeatureDataset_prior(os.path.join(path, 'train'))
        self.test = MSCOCOFeatureDataset_prior(os.path.join(path, 'val'))
    ...
    def fid_stat(self):
        # return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_MIMIC_test.npz'
        return f'/storage/ScientificPrograms/Diffusion/code/U-ViT-main/assets/fid_stats/fid_stats_Colorectal.npz'
...
def get_dataset(name, **kwargs):
    ...
    elif name == 'nlmcxr256_features':
        return NLMCXR256Features(**kwargs)
    elif name == 'mimic256_features':
        return MIMIC256Features(**kwargs)
    elif name == 'mimic256_features_prior':
        return MIMIC256Features_prior(**kwargs)
    elif name == 'mimic256_features_fine':
        return MIMIC256Features_fine(**kwargs)
    elif name == 'mimic256_features_test':
        return MIMIC256Features_test(**kwargs)
    elif name == 'mimic256_features_CL':
        return MIMIC256Features_CL(**kwargs)
    elif name == 'ISIC256_features_prior':
        return ISIC256Features_prior(**kwargs)
    elif name == 'Colorectal256_features_prior':
        return Colorectal256Features_prior(**kwargs)
    else:
